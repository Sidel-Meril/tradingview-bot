...
<b>Пользователь должен разрешить просматривать свой профиль с пересланных сообщений.</b>

Операции с оплатой
/accept - <i>принять оплату пользователя и уведомить его об этом</i>
Например, <code>/accept {user_id}</code> примет оплату у Вас и уведомит об этом вас. 
/decline - <i>отклонить оплату</i>
<code>/decline {user_id}</code> отправит Вам сообщение, что Ваш запрос на оплату не принят.
<b>Примеры использования команд для принятия оплаты польователя, от которого поступила оплата, указаны в сообщении с оплатой пользователя.</b>
/gift - <i>подарить подписку</i>
<code>/gift {user_id} 3</code> подарит подписку Вам на 3 дня. 
Если пользователь уже есть в списке /paid, длительность его подписки будет сокращена до указанного количества дней и будет начинаться от момента использования указанной команды.
    """
...
"""
    ...
    ALARM = '<b>До окончания Вашей подписки остался 1 день!</b>. \n<i>После {date_str} Вы не сможете использовать платные функции бота... 👉/request или обратитесь к нам, если произошла ошибка /ask</i>'
    MESSAGE_FROM_USER = """Сообщение от пользователя <a href="tg://user?id={user_id}">пользователя</a> <code>{user_id}</code>
<i>"{message}"</i>

Чтобы ответить, введите <code>/answer {user_id}</code> 
    """
...
"""
    ...
    PAY_GIFTED = """🎁 <b>Администратор подарил Вам подписку на {days} дня(-ей)</b>. Пробный период окончится <b>{date_str}</b>. Напоминаем, что ознакомиться с условиями подписки можно через команду /pay, когда длительность пробной подписки истечет или спросите у администратора об условиях оплаты напрямую через /ask."""